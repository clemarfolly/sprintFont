...
4. 添加完蛇形线后，因为圆弧使用若干线段模拟，所以最终长度还是会有偏差，可以先通过“偏差-”按钮来自动最小化偏差，如果结果不满意，还可以使用“目标偏差”参数来调整实际长度和期望长度的偏差值，“目标偏差”可以为正数或负数    
...
3. 软件支持中英文界面，启动时自动识别系统语种，如果需要手动切换，可以在config.json里面修改 ("language":"en/zh-ch/de/es/pt/fr/ru/tr")    

4. 软件自动将最近5个操作的输入文件备份到 `Users\name\AppData\Roaming\sprintFont` 目录，如果中间出现什么错误，可以使用Sprint-Layout直接导入对应的备份文件即可恢复你之前的工作。  

5. 如果要输入特殊符号，建议使用以下这些字体
**DejaVu Sans, Noto Sans, Symbola, Segoe UI Symbol, Arial Unicode MS**, ...   

6. 如果插件无法执行，请下载安装VC运行库2015/2017/2019/2022（任意一个版本均可），如果还不行，可能还需要“Windows 通用 C 运行库（Universal C Runtime）”，下载链接可以自己搜索，或者直接使用下面的链接：    
...
### v1.9
  1. 添加 "批处理修改" 功能
  2. 导出到kicad支持网表
  3. 精确的差分线对匹配功能，现在一次点击就可以自动匹配差分线长度偏差到0.001mm
  4. 保留历史备份文件（默认最近5个）到配置目录
  5. 添加方便输入的特殊符号列表和文本输入历史记录

### v1.8.1
  1. 添加导出为SVG功能
  2. 更智能的圆弧导线, 可以将多段首尾相连的导线视为一根而添加圆弧
  3. bugfix: 修正v1.8引入的"添加泪滴焊盘失败"bug

### v1.8
  1. 添加导出到kicad和OpenSCAD功能

### v1.7.1
  1. 配置文件保存到当前用户的AppData目录
